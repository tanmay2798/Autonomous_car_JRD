...
def mainp(frame):
    ...
    pub2 = rospy.Publisher('/carla/ego_vehicle/twist_cmd', Twist)
    msg = CarlaEgoVehicleControl()
    msg2 = Twist()
    msg2.linear.x = 1.5
    #msg.throttle = 0.22
    frame = frame[200:600,150:650]
    ...
    if predicted is not None and l2 is not None:
        ...
        x1=(predicted[0][2]+predicted[1][0])/2
	...
	#cv2.circle(frame, (int(x1),int(y)),5,(255,255,0),-1)
       ...
       # print(arr2)
        ...
        #if(dist==-1):
        deg = (round(-1*math.degrees(math.atan(dire)),2)/30)
        #print(deg)
        deg = pid(deg)
        deg=deg/2
        #sum1=sum1+
