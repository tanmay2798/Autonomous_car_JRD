...
def mainp(frame):
    ...
    #print(dt)
    #ret, frame = cap.read()
    pub = rospy.Publisher('/steer', Float32)
    ...
    if predicted is not None and l2 is not None:
      ...
      #  print(int(x))
        ...
        deg = (round(-1*math.degrees(math.atan(dire)),2))
        deg = pid(deg)

        if(dist==-1):
            print(deg)
            pub.publish(deg)
            cv2.putText(frame , str(deg), (20,50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 0, 0), 2, cv2.LINE_AA)
        else:
            print(0)
            pub.publish(0)
